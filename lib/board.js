...
var Board = function (options) {
  ...
  this.serialport = options && options.serialport || false;
  this.writeBuffer = [];

  var self = this;

  var initBoard = function (err, serial) {
    if (err) {
  ...
  if (this.serialport) {
    this.connectDirectly(this.serialport, initBoard);
  } else {
    this.detect(initBoard);    
  }
}
...
/*
 * connect an Arduino board - used in case the port is provided
 */

Board.prototype.connectDirectly = function(portName, callback) {
  this.log('info', 'attempting to connect to Arduino board on port: ' + portName);
  var self = this;
  var found = false;
  var temp;
  var err = null;
  try {
    temp = new serial.SerialPort('/dev/' + portName, {
      baudRate: self.baudrate,
      parser: serial.parsers.readline('\n')
    });
  } catch (e) {
  ...
  if(!err) {
    found = temp;
    this.log('info', 'found board at ' + found.path);
  } else {
    err = new Error('Could not find Arduino');
  }
  callback(err, found);
